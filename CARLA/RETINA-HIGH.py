...
# Baseline: always use the highest-accuracy model in the portfolio
FIXED_MODEL_NAME = 'ShuffleNetV2x1.0'
FIXED_MODEL_ID   = next(
...
def main():
    ...
    try:
        ...
        record_path  = os.path.join(
            RECORD_DIR,
            time.strftime("retina_high_%Y%m%d_%H%M%S.avi")
        )
        ...
        log_path   = os.path.join(LOG_DIR, f"retina_high_{run_tag}.csv")
        detail_log_path = os.path.join(LOG_DIR, f"retina_high_{run_tag}.jsonl")
        ...
        cv2.namedWindow(
            "CARLA - RETINA-High (Fixed ShuffleNet-1.0) Baseline",
            cv2.WINDOW_AUTOSIZE
        )

        print("\n" + "="*65)
        print(f"  SIMULATION RUNNING — RETINA-High fixed-model baseline")
